...
assert.match(html,/\.\/js\/app\.js\?v=app-v70/,'HTML must request the current app bundle');
assert.match(html,/\.\/css\/app\.css\?v=login-v2/,'HTML must request the login-preview stylesheet by a new URL');
assert.match(worker,/const VERSION="suphan-pwa-v194"/,'service worker cache must be replaced for this release');
assert.match(worker,/\.\/js\/app\.js\?v=app-v70/,'offline shell must match the new app bundle');
...
assert.match(app,/service-worker\.js\?v=146/,'the browser must check the updated service worker');
