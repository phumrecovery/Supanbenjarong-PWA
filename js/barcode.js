...
export function renderBarcode(root,api,session,goBack,{toast=()=>{}}={}){
  ...
  const current=()=>root.isConnected&&root.dataset.route==="barcode";
  ...
  const resultHtml=()=>{
    const q=state.query.trim().toLocaleLowerCase("th-TH");if(!q)return "";
